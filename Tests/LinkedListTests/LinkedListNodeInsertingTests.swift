...
final class LinkedListNodeInsertingTests: XCTestCase {
    ...
    func testInsertBeforeOnEmptyList() throws {
        ...
        let nodeForInsert = Node(item: itemForInsert)
        vts.insert(itemForInsert, at: 0)
        sut.insert(node: nodeForInsert, beforeNode: sut.head)
    ...
    func testInsertAfterOnEmptyList() throws {
        ...
        let nodeForInsert = Node(item: itemForInsert)
        vts.insert(itemForInsert, at: 0)
        sut.insert(node: nodeForInsert, afterNode: sut.head)
    ...
    func testInsertBeforeHeadTests() throws {
        ...
        sut.insert(node: Node(item: vts[2]), beforeNode: sut.head)
        sut.insert(node: Node(item: vts[1]), beforeNode: sut.head)
        sut.insert(node: Node(item: vts[0]), beforeNode: sut.head)
    ...
    func testInsertAfterHeadTests() throws {
        vts = ["This is a string", "This is another string", "Oh a third one!"]
        sut.insert(node: Node(item: vts[0]), afterNode: sut.head)
        sut.insert(node: Node(item: vts[2]), afterNode: sut.head)
        sut.insert(node: Node(item: vts[1]), afterNode: sut.head)
    ...
    func testInsertBeforeTailTests() throws {
        vts = ["This is a string", "This is another string", "Oh a third one!"]
        sut.insert(node: Node(item: vts[2]), beforeNode: sut.tail)
        sut.insert(node: Node(item: vts[0]), beforeNode: sut.tail)
        sut.insert(node: Node(item: vts[1]), beforeNode: sut.tail)
    ...
    func testInsertAfterTailTests() throws {
        vts = ["This is a string", "This is another string", "Oh a third one!"]
        sut.insert(node: Node(item: vts[0]), afterNode: sut.tail)
        sut.insert(node: Node(item: vts[1]), afterNode: sut.tail)
        sut.insert(node: Node(item: vts[2]), afterNode: sut.tail)
    ...
    func testPushFront() throws {
        ...
        sut.pushFront(node: Node(item: insertItem))
        try XCTAssertList(sut, matches: vts)
    ...
    func testPushBack() throws {
        var insertItem = "This is a test string"
        vts.append(insertItem)
        sut.pushBack(node: Node(item: insertItem))
        try XCTAssertList(sut, matches: vts)
